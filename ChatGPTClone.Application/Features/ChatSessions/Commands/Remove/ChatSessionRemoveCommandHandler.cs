...
public class ChatSessionRemoveCommandHandler : IRequestHandler<ChatSessionRemoveCommand, ResponseDto<Guid>>
{
    ...
    public ChatSessionRemoveCommandHandler(IApplicationDbContext dbContext, ICurrentUserServices currentUserService, IMemoryCache memoryCache)
    {
    ...
    {
        var chatSession = await _dbContext.ChatSessions.FirstOrDefaultAsync(x => x.Id == request.Id);
